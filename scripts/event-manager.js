...
/* 
    event-manager.js
    version 0.3.1 (only an early draft!)
...
let nativeAddEvent = EventTarget.prototype.addEventListener,
    nativeRemoveEvent = EventTarget.prototype.removeEventListener;
...
EventTarget.prototype.addEventListener = function(type, listener, ...options) {
    ...
    nativeAddEvent(type, listener, options);
    let done = false;
    ...
    if (type === null && listener === null) {
        for (ev in __eventFunctions) {
            for (func in __eventFunctions[ev]) {
                nativeRemoveEvent(ev, __eventFunctions[ev][func]);
            } 
        }
        this.__events.clear();
    } else if (listener === null) {
        for (func in __eventFunctions[type]) {
            nativeRemoveEvent(type, __eventFunctions[type][func]);
        }
        this.__events.delete(type);
    } else {
        nativeRemoveEvent(type, listener, options);
        for (const[k, v] of this.__events.entries()) {
            if (k == type) {
